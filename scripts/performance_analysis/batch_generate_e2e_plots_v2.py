...
def import_csv_results(root_dir, folder_prefix, data_type):
    ...
    for run_dir in run_dirs:
        # Extract run number from directory name. Robust to any prefix/name
        # shape (old convention was "<prefix>R1", ours is "<prefix>1_results") --
        # just pull the first digit run of characters and prefix with "R" so the
        # rest of the script's "R1"/"R2" sorting logic keeps working unchanged.
        ...
        for csv_file in csv_files:
            ...
            # Older naming has more text after the type (e.g. "_vehicle_2024...");
            # ours ends right at the type before .csv (e.g. "_vehicle.csv") -- accept either.
            data_type_lower = data_type.lower()
            if (data_type_lower + "_") not in csv_file and not csv_file.lower().endswith(data_type_lower + ".csv"):
                print(f'\tSkipping file that does not contain {data_type_lower}')
                continue
...
def generate_all_dest_all_runs_hist_plot(plots_dir,data_type,run_data_frames,plot_styles,all_source_sites,all_destination_sites,max_bin_value,annotate):
    ...
    for source_site in all_source_sites:
        ...
        # With 3+ destination sites, ax.hist() returns a list of BarContainers
        # (one per dataset). With exactly 1 destination (e.g. a 2-site setup),
        # it collapses and returns a single bare BarContainer instead. Note:
        # BarContainer itself subclasses tuple, so isinstance(x, tuple) is
        # ALWAYS True and can't distinguish the two cases -- use the known
        # number of datasets we passed in instead, which is unambiguous.
        ...
        for i,data in enumerate(list_of_all_dest_clipped):
            # Axes.ecdf() only exists in matplotlib>=3.8 (needs Python 3.9+),
            # which isn't compatible with this environment's Python 3.7 --
            # compute the empirical CDF manually and draw it as a step plot instead.
...
def main():
    ...
    plot_performance_data("results", "Run", "Vehicle", True)
    plot_performance_data("results", "Run", "J2735-BSM", True)
    return

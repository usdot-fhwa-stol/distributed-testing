...
$VUG_JSON_STREAMER_PATH/start.sh $useBestEffort -emEndpoints $VUG_EM_ADDRESS:$VUG_EM_PORT -verbosity $adapterVerbosity | awk -v adapter="[$VUG_JSON_STREAMER_VERSION]" '{print adapter, $0, fflush(); }' | tee -a $adapterLogFile
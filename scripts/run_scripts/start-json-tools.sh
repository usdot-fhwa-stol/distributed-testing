...
adapterLogFile=$VUG_ADAPTER_LOG_PATH/json_tools_terminal_out.log
...
$VUG_JSON_TOOLS_PATH/start.sh $useBestEffort -emEndpoints $VUG_EM_ADDRESS:$VUG_EM_PORT -verbosity $adapterVerbosity | awk -v adapter="[$VUG_JSON_TOOLS_VERSION]" '{print adapter, $0, fflush(); }' | tee -a $adapterLogFile